...
public class Raycast : MonoBehaviour
{
    ...
    public void Generate(int numberOfRays, int fov, int viewRange)
    {
        ...
        this.angles = new double[numberOfRays];

        Physics2D.queriesStartInColliders = false;

        double initAngle = 90 - fov / 2;
        double step = fov / (numberOfRays-1);
        print("Init angle " + initAngle);
        for (int i = 0; i < numberOfRays; i++)
        {
            double angle = i * step + initAngle;// - fov/2; //+ (fov/numberOfRays)/2;
            float angleInRadians = (float)angle * Mathf.Deg2Rad;
            print("i " + i +  " step " + angle );
