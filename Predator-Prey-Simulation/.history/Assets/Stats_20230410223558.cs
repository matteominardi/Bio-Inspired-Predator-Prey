    ...
    void Update()
    {
        ...
        if (hit.collider != null)
        {
            if (Input.GetMouseButtonDown(0) && hit.transform.tag == "SelectableObject")
            {
                ...
                gameObject.transform.Find("lblHealth").transform.Find("txtHealth").GetComponent<TMPro.TextMeshPro>().text = health
                print(gameObject.transform.Find("lblHealth").transform.Find("txtHealth").GetComponent<TMPro.TextMeshPro>().text);//.transform.Find("txtHealth").GetComponents<MonoBehaviour>());
